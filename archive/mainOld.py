...
class Move(object):
    def __init__(self, from_, to, piece, promotion = None, castle = None):
        self.from_ = from_
        self.to = to
        self.piece = piece
...
def count_bits(bb):
    ...
    for i in range(64):
        if bb == 0: return count
        right_bit_index = get_right_bit_index(bb, start)
        start = right_bit_index+1
        bb = bb & ~Square(right_bit_index).toBoard()
        count += 1
    return count
...
class Board(object):
    def __init__(self):
        ...
        self.all_pieces_per_color = np.zeros(2, dtype=np.uint64)  # Combined bitboard for all pieces of given side
        self.all_pieces = np.uint64(0)  # Combined bitboard for all pieces on the board
...
def get_legal_moves(board):
    ...
    for piece in Piece:
        ...
        while piece_bb:
            ...
            start = right_bit_index + 1
            list_of_moves += get_legal_moves_from(piece, board, right_bit_index)
